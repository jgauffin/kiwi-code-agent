...
- The transcript shows a question card per request: one group per question, single or multi select, free-text "Other". Submit resolves the request; the session status is `needs_human` until then.
