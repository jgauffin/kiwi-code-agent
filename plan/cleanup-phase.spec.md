...
- **B11**: however the run ends — of its own accord or because the user stopped it — the files are measured again and the test run repeats, and the plan bar line carries both outcomes. (docs/features/cleanup-phase.md#Cleanup phase)
...
- **B12**: a test run that fails after the cleanup goes to the implement session like any other failure, and repeats until the board is green again; no such pass starts another cleanup. (docs/features/cleanup-phase.md#Cleanup phase)
